      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
              ...
              switch (widget.fieldType) {
                case 'email':
                  ...
                  if (_controller.text == widget.oldValue) {
                    ...
                    await widget.cubit.changeEmail(_controller.text);
                    if (!context.mounted) return;
                    if (widget.cubit.state.loadStatus == LoadStatus.Success) {
                      showDialog(
                        context: context,
                        builder:
                    ...
                    } else if (widget.cubit.state.loadStatus ==
                        LoadStatus.Error) {
                      showMyDialog(
                        context,
                        'Thông báo',
                ...
                case 'password':
                  ...
                  if (_controller.text == widget.oldValue) {
                    ...
                    await widget.cubit.changePassword(_controller.text);
                    if (!context.mounted) return;
                    if (widget.cubit.state.loadStatus == LoadStatus.Success) {
                      showMyDialog(
                        context,
                ...
                default:
                  ...
                  if (_controller.text == widget.oldValue) {
                    ...
                    await widget.cubit.changeUsername(_controller.text);
                    if (!context.mounted) return;
                    if (widget.cubit.state.loadStatus == LoadStatus.Success) {
                      showMyDialog(
                        context,
            ...
            child: Text('Lưu', style: TextStyle(color: AppColors.primaryDark)),
          ),
        ],
      ),
      body: Padding(
        ...
        child: Column(
          children: [
            widget.fieldType == 'username'
                ? const Text(
                  'Tên người dùng chỉ được đổi duy nhất sau lần gần nhất 60 ngày (còn 00 ngày).',
                )
